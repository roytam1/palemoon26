...
// If spec compliant pref isn't set, pretend we never got these headers. 
  ...
  //
  // In general, we should not be setting CSP on a principal that aliases another
  // document. For non-app code this is not a problem, since we only share the
  // underlying principal with nested browsing contexts for which a header cannot
  // be set (e.g., about:blank and about:srcdoc iframes), and thus won't try to set
  // the CSP again. This check ensures that we don't try to set CSP for an app.
  if (applyAppDefaultCSP || applyAppManifestCSP) {
    ...
    if (csp) {
      ...
      PR_LOG(gCspPRLog, PR_LOG_DEBUG, ("%s %s %s",
           "This document is sharing a principal with another document.",
